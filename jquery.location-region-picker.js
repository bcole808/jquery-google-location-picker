...
;(function ( $, window, document, undefined ) {
	...
	LocationRegionPicker.prototype.newLocationSelected = function() {
	  ...
	  if (country) {
		  $details.append('Country: <input type="text" name="country" value="'+country+'" disabled />');
		}

		var locality = this.getDetail('locality');
	  if (locality) {
		  $details.append('Loaclity: <input type="text" name="locality" value="'+locality+'" disabled />');
		}

		if (this.place.url) {
			$details.append('Map: <input type="text" name="map_url" value="'+this.place.url+'" disabled />');
		}

		if (this.place.geometry && this.place.geometry.location) {
			$details.append('Lat: <input type="text" name="latitude" value="'+this.place.geometry.location.lat()+'" disabled />');
			$details.append('Lon: <input type="text" name="longitude" value="'+this.place.geometry.location.lng()+'" disabled />');
		}
